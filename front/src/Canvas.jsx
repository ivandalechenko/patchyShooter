...
const SpriteAnimation = ({ shotTrigger }) => {
    ...
    useEffect(() => {
        if (shotTrigger && shotTrigger !== lastShotTrigger.current) {
            ...
            oldBloodCount.current = ((oldBloodCount.current + 1) % 2)
            if (bloodRef.current) {
                bloodRef.current.x(getX() + [0, -100, 0][oldBloodCount.current])
                bloodRef.current.y(getY() + [0, 300, -500][oldBloodCount.current])
                bloodRef.current.rotation([0, -25, 40][oldBloodCount.current])
